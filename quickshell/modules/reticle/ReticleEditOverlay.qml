...
// Full-screen drag/resize UI for the aiming reticle - reuses the exact
// interaction shape as modules/desktop/DesktopEditOverlay.qml (drag to
// reposition, corner handles to scale uniformly, snap to screen
// edges/center), swapped to drive ReticleStore/ReticleView instead of
// DesktopOverlayStore/ClockWeatherCard. Also carries the reticle-specific
// controls (style/color cycling) that the clock card didn't need.
PanelWindow {
    id: root

    // Must be the same output as ReticleWindow.qml - the proxy's x/y here
    // are normalized against this window's own width/height, so a
    // mismatched screen would save a position that looks right here but is
    // wrong once the live overlay (on the actual primary output) picks it
    // back up.
    screen: MonitorRoles.primaryScreen()

    WlrLayershell.layer: WlrLayer.Top
    WlrLayershell.namespace: "quickshell:reticle:edit"
    ...
    Shortcut {
        sequence: "Escape"
        onActivated: ReticleStore.setEditMode(false)
    }
    ...
    // ReticleWindow.qml's sizing logic so what you drag here matches what
    // actually renders once you exit edit mode.
    //
    ...
    Item {
        ...
        property real _nx: ReticleStore.getPos().nx
        property real _ny: ReticleStore.getPos().ny
        ...
        // (mirrors ReticleStore.scale normally, this override only while
        // actively resizing) instead of getting permanently detached from
        // the store the first time it's imperatively assigned to.
        property real _liveResizeScale: 1.0
        readonly property real _previewScale: proxy._resizing ? proxy._liveResizeScale : ReticleStore.scale
        ...
        DragHandler {
            ...
            onActiveChanged: {
                if (dragger.active) {
                    ...
                    ReticleStore.setPos(proxy._nx, proxy._ny);
                }
            }
        ...
        // aspect ratio is locked (ReticleView is always square), so there's
        // only one degree of freedom (scale) regardless of which handle is
        // grabbed. Dragging any of them scales the whole reticle uniformly
        // from its own center, by the ratio of the pointer's current
        // distance from center to its distance when the drag started.
        Repeater {
                ...
                DragHandler {
                    ...
                    onActiveChanged: {
                        if (resizeDrag.active) {
                            ...
                            resizeDrag._startScale = ReticleStore.scale;
                            ...
                            ReticleStore.setScale(proxy._previewScale);
                            ReticleStore.setPos(proxy._nx, proxy._ny);
                    ...
                    onCentroidChanged: {
                        ...
                        proxy._liveResizeScale = Math.max(ReticleStore.minScale, Math.min(ReticleStore.maxScale, resizeDrag._startScale * (curDist / resizeDrag._startDist)));
                    }
                }
            }
        }
    ...
    Row {
        ...
        // Cycles ReticleStore.style - click through Cross/Dot/Circle/Chevron
        // without leaving edit mode, so you can compare styles at the
        // position/scale you're currently dragging.
        Rectangle {
            implicitWidth: styleLabel.implicitWidth + 28
            implicitHeight: styleLabel.implicitHeight + 16
            radius: height / 2
            color: styleHover.hovered ? Theme.accent : Theme.pillBg
            ...
            Text {
                id: styleLabel
                anchors.centerIn: parent
                text: "Style: " + ReticleStore.style
                color: styleHover.hovered ? Theme.onAccentText : Theme.text
                font.family: Theme.fontFamily
                font.pixelSize: Theme.fontSize - 2
            ...
            TapHandler {
                onTapped: ReticleStore.cycleStyle()
            }
        }

        // Toggles the dark contrast stroke drawn behind every shape - see
        // ReticleView.qml's outlineWidth. Off gives a slightly cleaner look
        // on consistently dark backgrounds; on keeps the reticle visible
        // over arbitrary bright game content.
        Rectangle {
            implicitWidth: outlineLabel.implicitWidth + 28
            implicitHeight: outlineLabel.implicitHeight + 16
            radius: height / 2
            color: ReticleStore.outlined ? Theme.withAlpha(Theme.accent, 0.18) : Theme.pillBg
            border.color: ReticleStore.outlined ? Theme.accent : "transparent"
            border.width: 1
            ...
            Text {
                ...
                anchors.centerIn: parent
                text: "Outline"
                color: ReticleStore.outlined ? Theme.accent : Theme.dimText
                font.family: Theme.fontFamily
                font.pixelSize: Theme.fontSize - 2
            }
            ...
            HoverHandler {}
            TapHandler {
                onTapped: ReticleStore.toggleOutlined()
            }
        }

        // Cycles ReticleStore.color through the high-contrast palette.
        Rectangle {
            implicitWidth: colorLabel.implicitWidth + colorSwatch.width + 36
            implicitHeight: colorLabel.implicitHeight + 16
            radius: height / 2
            color: colorHover.hovered ? Theme.accent : Theme.pillBg
            ...
            Row {
                ...
                Text {
                    ...
                    anchors.verticalCenter: parent.verticalCenter
                    text: "Color"
                    color: colorHover.hovered ? Theme.onAccentText : Theme.text
                    font.family: Theme.fontFamily
                    font.pixelSize: Theme.fontSize - 2
                    font.weight: Font.Medium
                }
            ...
            TapHandler {
                onTapped: ReticleStore.cycleColor()
            }
        }
        ...
        Rectangle {
            ...
            TapHandler {
                onTapped: ReticleStore.setEditMode(false)
            }
        }
    }
}
